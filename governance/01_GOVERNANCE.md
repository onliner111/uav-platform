...
| --- | --- | --- |
| Engineering constitution | `governance/AGENTS.md` | Engineering constraints and guardrails only. |
| Product milestones | `governance/ROADMAP.md` | What to deliver and milestone outcomes only. |
| Execution playbook | `governance/EXECUTION_PLAYBOOK.md` | How to execute and verify delivery only. |
...
| Documentation governance | `governance/01_GOVERNANCE.md` | SSOT boundaries and change rules only. |
...
4. `governance/AGENTS.md`, `governance/ROADMAP.md`, and `governance/EXECUTION_PLAYBOOK.md` must not contain phase task definitions.
5. New phases should keep the two-digit grouping convention (01-12 model) used by `governance/00_INDEX.md`.

## 3. Engineering Rule Change Rules

1. Engineering policy changes must be made in `governance/AGENTS.md`.
...
4. If a change affects verification gates, update `governance/EXECUTION_PLAYBOOK.md` in the same change set.

## 4. Execution Rule Change Rules

1. Execution command/workflow changes must be made in `governance/EXECUTION_PLAYBOOK.md`.
2. `governance/EXECUTION_PLAYBOOK.md` must not define product milestones or phase acceptance requirements.
...
Required artifacts:
1. Regenerate `docs/gpt_SYSTEM_SNAPSHOT.md` via `tooling/gpt/gen_report1.sh`.
2. Regenerate `docs/gpt_ARCHITECTURE_RISK_REPORT.md` via `tooling/gpt/gen_report2.sh`.
